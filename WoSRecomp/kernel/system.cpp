...
PPC_FUNC(__imp__KeDelayExecutionThread)
{
    ...
    // Same rule as the object waits: an alertable delay that delivers an APC
    // ends early with STATUS_USER_APC rather than sleeping out its interval.
    // Routed through the shared helper so this site's Alertable value is
    // reported in the same place and the same format as every other wait.
    if (wos::AlertableReturn(ctx, base, ctx.r4.u32, "KeDelayExecutionThread"))
        return;
